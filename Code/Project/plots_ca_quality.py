...
CA-vs-Quality plotting utility for similarity-cache benchmarks.
- Un grafico per *ogni threshold* (fissata la *capacity*).
- Asse X = qualità (media coseno sui hit) -> colonna: `avg_similarity`
- Asse Y = costo teorico `C_A` -> colonna: `C_A`
- Legenda: policy + parametri (se presenti) + **HR=...** (+ opzionale hint tempo GPU) e CA
- Evidenzia SOLO la run "migliore" (marker più grande, bordo, annotazione con ★ e prefisso ★ BEST · in legenda).
- Criteri selezione "best" (adattati a: max quality, **min** C_A):
    - pareto_knee (default): scelto sulla Pareto front (max X, min Y) con minima distanza da (1, 0) dopo normalizzazione di Y
    - distance        : minima distanza euclidea da (1, 0) dopo normalizzazione di Y
    - product         : massimizza  x * (1 - y_norm)
    - hmean           : massimizza  Hmean(x, 1 - y_norm)
    - lexi            : ordina per quality desc, poi C_A asc

Input atteso (cartella benchmark):
- summary.csv    (obbligatorio)
- manifest.json  (opzionale, per leggere num_requests per l'hint GPU)
- histories/     (non usato qui)
...
Uso (programma):
    from plots_ca_quality import plot_all_thresholds
    plot_all_thresholds(
        ...
        save_dir=None,            # default: <bench_dir>/charts_ca_quality
        annotate_best=True,
        best_rule="pareto_knee",
    )

Uso (CLI):
    python -m plots_ca_quality \
      --bench_dir /path/to/Results/<stamp> \
      --capacity 200 \
...
def _legend_label(row: pd.Series,
    ...
    ca_str = f"CA={ca_val:.3g}" if isinstance(ca_val, (int, float, np.floating)) else "CA=n/a"
    hr_str = f"HR={_fmt_hr(row)}"
    gpu_hint = _fmt_gpu_time(row, gpu_name, miss_cost_s, hit_cost_s, num_requests)

    base = pol
    if params:
        base += f" ({params})"
    base += f" — {ca_str}, {hr_str}{gpu_hint}"
    return base


# ---------- Best selection (max quality, min C_A) ----------

def _pareto_front_min_y(df: pd.DataFrame, x_col: str, y_col: str) -> pd.DataFrame:
    """
    Restituisce i punti non-dominati quando vogliamo massimizzare x e MINIMIZZARE y.
    """
    sub = df.copy().sort_values([x_col, y_col], ascending=[False, True])
    out = []
    best_y = float("inf")
...
def _choose_best(sub: pd.DataFrame, method: str = "pareto_knee") -> int:
    """
    Ritorna l'indice *iloc* della riga 'migliore' in sub (x=avg_similarity MAX, y=C_A MIN).
    Metodi:
      - 'pareto_knee' (default): punto sulla Pareto front (max X, min Y) con minima distanza da (1, 0)
      - 'distance'    : minima distanza euclidea da (1, 0) dopo normalizzazione di Y
      - 'product'     : massimizza  x * (1 - y_norm)
      - 'hmean'       : massimizza  Hmean(x, 1 - y_norm)
      - 'lexi'        : ordina per quality desc, poi C_A asc
    ...
    if "avg_similarity" not in subn.columns or "C_A" not in subn.columns:
        raise KeyError("summary.csv deve contenere le colonne 'avg_similarity' e 'C_A'")
    x = subn["avg_similarity"].astype(float).values
    y = subn["C_A"].astype(float).values

    # Normalizzazione y -> [0,1] per metodi che richiedono distanza/combinazioni
    ...
    if method == "hmean":
        ...
        a = x
        b = 1.0 - y_norm
        scores = 2.0 / (np.maximum(eps, 1.0/a) + np.maximum(eps, 1.0/b))
        return int(np.argmax(scores))

    if method == "pareto_knee":
        # Pareto front (max x, min y), poi distanza da (1, 0) in (x, y_norm) space
        pf = _pareto_front_min_y(subn.assign(y_norm=y_norm), "avg_similarity", "y_norm")
        if len(pf) == 0:
            method = "distance"
        else:
            xs = pf["avg_similarity"].astype(float).values
            ys = pf["y_norm"].astype(float).values
            d2 = (1.0 - xs) ** 2 + (0.0 - ys) ** 2
            best_idx_in_pf = int(np.argmin(d2))
            return int(pf.index[best_idx_in_pf])

    # default: 'distance' (dopo normalizzazione y_norm)
    d2 = (1.0 - x) ** 2 + (0.0 - y_norm) ** 2
...
def _filter_capacity_threshold(df: pd.DataFrame, capacity: float, threshold: float) -> pd.DataFrame:
    # tolleranza su float per threshold; capacity spesso è int ma può essere float
    df2 = df.copy()
...
def plot_for_threshold(
    ...
    ax.set_ylabel("C_A (theoretical cost)")
    ax.set_title(f"C_A vs Quality — capacity={capacity}, threshold={threshold}")
    ax.legend(loc="center left", bbox_to_anchor=(1.02, 0.5), fontsize=14, frameon=False)
    ax.grid(True, linestyle="--", alpha=0.3)

    _ensure_dir(out_dir)
    out_path = out_dir / f"ca_vs_quality_capacity{capacity}_thr{threshold}.png"
    fig.tight_layout()
...
def plot_all_thresholds(
    ...
    for col in ("capacity", "threshold", "avg_similarity", "hit_rate", "C_A"):
    ...
    # cartella output
    save_dir = Path(save_dir) if save_dir is not None else (bench_dir / "charts_ca_quality")
...
def _parse_cli(argv: Optional[List[str]] = None) -> dict:
    import argparse
    ap = argparse.ArgumentParser(description="Plot C_A vs Quality per-threshold per una capacity (con evidenziazione best).")
    ...
    ap.add_argument("--save_dir", type=str, default=None, help="Dove salvare i PNG (default: <bench_dir>/charts_ca_quality)")
    ap.add_argument("--best_rule", type=str, default="pareto_knee",
                    choices=["pareto_knee", "distance", "product", "hmean", "lexi"],
                    help="Criterio di scelta del best run (max quality, min C_A)")
