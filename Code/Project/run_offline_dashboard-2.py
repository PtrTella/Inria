...
def make_trace(manager, cfg):
    mode = cfg.get("trace_mode", "sequential")
    n = int(cfg.get("num_requests", 2000))
    if mode == "sequential":
        idx = manager.sample_prompts(num_prompts=n, random_order=False)
    elif mode == "random":
        idx = manager.sample_prompts(num_prompts=n, random_order=True)
    elif mode == "sessions":
        ...
        idx = np.concatenate([ensure_index_array(i) for (i, _) in sess]) if len(sess) else np.array([], dtype=int)
...
def main():
    ...
    for cfg in todo:
        ...
        sim.save_history(path=parquet_path, format="parquet")
